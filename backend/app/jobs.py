    ...
    for L in listings:
        ...
        if client:
            if client and client.email and client.email_opt_in:
                ...
                db.add(Notification(                                      # NEW: record intent to send
                    channel="email",
                    listing_key=str(L.get("ListingKey") or L.get("ListingId") or ""),
                    status="queued",
                    detail="gmail",
                    client_id=client.id,
...
def _run_saved_search_gpt(search: SavedSearch, db: Session) -> int:
    ...
    if email_items:
        ...
        to_send = [(i["to"], i["subject"], i["html"], i.get("text")) for i in email_items]
...
def _compose_email(listing: dict, analysis: dict, search: SavedSearch) -> tuple[str, str, str]:
